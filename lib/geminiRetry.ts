...
interface GeminiErrorInfo {
  // 재시도하면 성공할 가능성이 있는 일시적 오류 (429 rate limit, 503 overloaded 등)
  isTransient: boolean;
  // 재시도해도 절대 해결되지 않는 오류 (하루 요청 한도 소진)
  isDailyQuotaExceeded: boolean;
  // 모델이 일시적으로 과부하 상태인 경우 (503 UNAVAILABLE)
  isOverloaded: boolean;
  retryDelayMs: number | null;
}

// Gemini SDK가 던지는 에러의 message에는 REST API의 JSON 에러 본문이
// 그대로 문자열로 들어있는 경우가 많다 (429 RESOURCE_EXHAUSTED, 503 UNAVAILABLE 등).
export function parseGeminiError(error: unknown): GeminiErrorInfo {
  ...
  if (jsonMatch) {
    try {
      ...
      } else if (err.code === 503 || err.status === "UNAVAILABLE") {
        isTransient = true;
        isOverloaded = true;
      }
    } catch {
  ...
  if (!isTransient) {
    if (/503|UNAVAILABLE|overloaded|high demand/i.test(message)) {
      isTransient = true;
      isOverloaded = true;
    } else if (/429|RESOURCE_EXHAUSTED|rate.?limit/i.test(message)) {
      isTransient = true;
    }
  }

  return { isTransient, isDailyQuotaExceeded, isOverloaded, retryDelayMs };
}
...
export function toFriendlyGeminiErrorMessage(error: unknown): string {
  ...
  if (info.isOverloaded) {
    return "지금 Gemini 모델에 요청이 몰려 일시적으로 응답하지 못하고 있습니다. 잠시 후 다시 시도해주세요.";
  }
  if (info.isTransient) {
...
// 일시적인 오류(429 rate limit, 503 overloaded)에는 API가 알려준 재시도
// 대기시간(또는 지수 백오프)만큼 기다렸다가 자동으로 재시도한다.
// 하루 할당량 초과처럼 재시도해도 소용없는 에러는 즉시 그대로 던진다.
export async function generateContentWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      const info = parseGeminiError(error);
      if (!info.isTransient || info.isDailyQuotaExceeded || attempt === maxRetries) {
        throw error;
      }
